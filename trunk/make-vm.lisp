...
(defun make-vm (nom-vm taille)
  (let ((resultat (if (= 0 taille)
		    ...
		    (progn 
		      ...
		      (setf (get nom-vm ':R) '0)
		      (setf (get nom-vm ':DERNIERCODE) '0)
		      (setf (get nom-vm ':PTR) taille)
   ...
   ((aide (nom-vm ligne)
	  (if (< ligne (get nom-vm ':ram))
	    ...
	    (progn
		  ...
		  (write-line "")
		  (princ "derniere ligne de code : ")
		  (princ (get nom-vm ':DERNIERCODE))
		  (write-line "")
		  (princ "PTR : ")
		  (princ (get nom-vm ':PTR))
		  (write-line "")
	      (write-line "fin")
	      T))))
   (aide nom-vm 0)))
...
;; le resultat de la derniere operation sur la vm :
(defun get-resultat (vm)
	(if (eq (get vm ':R) '1)
		(progn  (write-line "")
				(princ " Machine Virtuelle : ")(princ vm)(write-line "")
				(princ "    OK ")(write-line "")
				T
		)
		(progn  (write-line "")
				(princ " Machine Virtuelle : ")(princ vm)(write-line "")
				(princ "    MOT NON RECONNU ")(write-line "")
				()
		)
		))
